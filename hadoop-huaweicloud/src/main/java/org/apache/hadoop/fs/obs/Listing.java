...
import static org.apache.hadoop.fs.obs.Constants.OBS_FOLDER_SUFFIX;
import static org.apache.hadoop.fs.obs.OBSUtils.createFileStatus;
import static org.apache.hadoop.fs.obs.OBSUtils.objectRepresentsDirectory;
import static org.apache.hadoop.fs.obs.OBSUtils.stringify;
import static org.apache.hadoop.fs.obs.OBSUtils.translateException;

/**
 * Place for the OBS listing classes; keeps all the small classes under control.
 */
public class Listing {
   ...
   * declaring that if the {@link OBSFileSystem.ObjectListingIterator#hasNext()}
  ...
  class FileStatusListingIterator
     ...
     * {@link OBSFileSystem.ObjectListingIterator} does not generate a request;
    ...
    private boolean buildNextStatusBatch(ObjectListing objects) {
      ...
      for (String prefix : objects.getCommonPrefixes()) {
        ...
        if (acceptor.accept(keyPath, prefix) && filter.accept(keyPath)) {
          FileStatus status = new OBSFileStatus(false, keyPath,
              owner.getUsername());
  ...
  /**
   * Wraps up OBS `ListObjects` requests in a remote iterator
  ...
  class ObjectListingIterator implements RemoteIterator<ObjectListing> {
     ...
     * remote IO. For later requests, OBS will be queried, hence the calls
     * may block or fail.
  ...
  /**
   * Accept all entries except the base path and those which map to OBS
  ...
  static class AcceptFilesOnly implements FileStatusAcceptor {
    ...
    public boolean accept(Path keyPath, ObsObject summary) {
      return !keyPath.equals(qualifiedPath)
          && !summary.getObjectKey().endsWith(OBS_FOLDER_SUFFIX)
          && !objectRepresentsDirectory(summary.getObjectKey(), summary.getMetadata().getContentLength());
    }
