...
public final class OBSUtils {
   ...
   */
  public static OBSFileStatus createFileStatus(Path keyPath,
                                               ObsObject summary,
                                               long blockSize,
                                               String owner) {
    if (objectRepresentsDirectory(summary.getObjectKey(), summary.getMetadata().getContentLength())) {
      ...
      return new OBSFileStatus(summary.getMetadata().getContentLength(),
          dateToLong(summary.getMetadata().getLastModified()), keyPath,
          blockSize, owner);
    }
  }
   ...
   * Handles null Dates that can be returned by OBS by returning 0
  ...
  /**
   * Return the access key and secret for OBS API use.
   ...
   */
  public static OBSLoginHelper.Login getOBSAccessKeys(URI name,
   ...
   * {@code fs.obs.bucket.${bucket}.key} to
   * {@code fs.obs.key}, for all values of "key" other than a small set
   ...
   *
   * The {@code fs.obs.impl} property cannot be set, nor can
   * any with the prefix {@code fs.obs.bucket}.
   *
   * This method does not propagate security provider path information from
   * the OBS property into the Hadoop common provider: callers must call
   ...
   * {@link Constants#OBS_SECURITY_CREDENTIAL_PROVIDER_PATH}.
   *
  ...
  static void patchSecurityCredentialProviders(Configuration conf) {
    ...
    if (!customCredentials.isEmpty()) {
      ...
      conf.set(CREDENTIAL_PROVIDER_PATH, joined,
          "patch of " + OBS_SECURITY_CREDENTIAL_PROVIDER_PATH);
    }
  }
 ...
 */
  ...
  public static String stringify(ObsException e){
    StringBuilder builder = new StringBuilder(
            String.format("request id: %s, response code: %d, error code: %s, hostid: %s, message: %s",
                    e.getErrorRequestId(),
                    e.getResponseCode(),
                    e.getErrorCode(),
                    e.getErrorHostId(),
   ...
   * HTTP error codes are examined and can be
   ...
   */
    ...
    public static IOException translateException(String operation,
     String path, ObsException exception)
    {
        ...
        switch (status)
        {
            case 301:
                message = String.format(
                            "Received permanent redirect response , message %s",
   ...
   * {@link ObsException} passed in, and any status codes included
